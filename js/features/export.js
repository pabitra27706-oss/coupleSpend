...
const ExportManager = (() => {
  ...
  // ── Export PDF ────────────────────────────
  async function exportPDF(type = 'monthly') {
    Toast.info('Generating', 'Creating PDF report...');
    ...
    try {
      ...
      addText(store.formatMonth(month), pageW - margin - 4, y + 10, {
        size: 12, style: 'bold',
        color: [255, 255, 255],
        align: 'right'
      });
      addText(`Generated: ${generatedAt}`, pageW - margin - 4, y + 18, {
        size: 8, color: [200, 200, 255], align: 'right'
      });
      y += 36;

      // ── Summary Section ──
      addText('Summary', margin, y, { size: 13, style: 'bold' });
      y += 8;
      addLine(y);
      y += 6;
      ...
      const boxes = [
        ...
        {
          label:  partner?.name || profile?.partnerName || 'Partner',
      ...
      boxes.forEach((box, i) => {
        ...
        addText(box.label, bx + 4, y + 8, {
          size: 9, style: 'bold', color: box.color
        });
        addText('Expenses', bx + 4, y + 16, {
          size: 7, color: [120, 120, 120]
        });
        addText(box.spend, bx + 4, y + 23, {
          size: 12, style: 'bold', color: [30, 30, 30]
        });
        addText(`Income: ${box.income}`, bx + boxW - 4, y + 23, {
      ...
      addText(
        `Combined: ${symbol}${(mySpend + herSpend).toFixed(2)}  ·  Difference: ${symbol}${Math.abs(diff).toFixed(2)} (${diff > 0 ? (partner?.name || 'Partner') + ' saved more' : (profile?.name || 'You') + ' saved more'})`,
        margin, y,
      ...
      // ── Category Breakdown ──
      checkPage(60);
      addText('Category Breakdown', margin, y, { size: 13, style: 'bold' });
      y += 8;
      addLine(y);
      y += 6;
      ...
      addText(profile?.name || 'You', cols.you, y + 4, {
        size: 8, style: 'bold', color: [99, 102, 241]
      });
      addText(partner?.name || 'Partner', cols.her, y + 4, {
      ...
      [...allCats]
        ...
        .forEach((catId, idx) => {
          ...
          addText(
            myAmt > 0 ? `${symbol}${myAmt.toFixed(2)}` : '-',
            cols.you, y + 3,
            { size: 8, color: myAmt > 0 ? [99, 102, 241] : [160, 160, 160] }
          );
          addText(
            herAmt > 0 ? `${symbol}${herAmt.toFixed(2)}` : '-',
            cols.her, y + 3,
            { size: 8, color: herAmt > 0 ? [236, 72, 153] : [160, 160, 160] }
          );
          addText(`${symbol}${total.toFixed(2)}`, cols.total, y + 3, {
      ...
      const addTxTable = (txns, ownerName, color) => {
        ...
        addText(`${ownerName}'s Transactions`, margin, y, {
          size: 13, style: 'bold'
        });
        y += 8;
        addLine(y);
        ...
        ['Date','Description','Category','Payment','Amount']
          .forEach((h, i) => {
            const x = [tc.date, tc.desc, tc.cat, tc.pay, tc.amt][i];
            addText(h, x, y + 4, {
        ...
        txns.slice(0, 50).forEach((tx, idx) => {
          ...
          addText(formatTxDate(tx.date), tc.date, y + 3, {
            size: 7, color: [100, 100, 100]
          });
          addText(
            (tx.description || getCatLabel(tx.category)).substring(0, 35),
            tc.desc, y + 3,
            { size: 7, color: [40, 40, 40] }
          );
          addText(getCatLabel(tx.category), tc.cat, y + 3, {
            size: 7, color: [100, 100, 100]
          });
          addText(tx.paymentMethod || '-', tc.pay, y + 3, {
            size: 7, color: [100, 100, 100]
          });
          addText(
            ...
            tc.amt, y + 3,
            {
              size:  7,
              style: 'bold',
              color: isExp ? [239, 68, 68] : [16, 185, 129],
              align: 'right'
            }
          );
          y += 9;
      ...
      doc.addPage();
      y = margin;
      addTxTable(myTxns, profile?.name || 'You', [99, 102, 241]);
      addTxTable(partnerTxns, partner?.name || 'Partner', [236, 72, 153]);
      ...
      for (let i = 1; i <= totalPages; i++) {
        ...
        addText('CoupleSpend · Finance Tracker', margin, 287, {
          size: 7, color: [150, 150, 150]
        });
        addText(`Page ${i} of ${totalPages}`, pageW - margin, 287, {
      ...
      doc.save(fileName);
      Toast.success('Exported', `PDF saved as ${fileName}`);

    } catch (err) {
      console.error('PDF export error:', err);
      Toast.error('Failed', 'Could not generate PDF');
    }
  }

  // ── Export Compare PDF ─────────────────────
  async function exportComparePDF() {
    return exportPDF('compare');
  }

  // ── Export CSV ────────────────────────────
  function exportCSV(who = 'both') {
    Toast.info('Generating', 'Creating CSV file...');

    try {
      const data = getExportData();
      const { myTxns, partnerTxns, symbol, month, profile, partner } = data;
      ...
      if (who === 'me') {
        ...
        txns = [
          ...myTxns.map(t => ({ ...t, _owner: profile?.name || 'You' })),
          ...partnerTxns.map(t => ({
            ...t,
            _owner: partner?.name || 'Partner'
          }))
      ...
      Toast.error('Failed', 'Could not generate CSV');
    }
  }

  // ── Export Excel ──────────────────────────
  function exportExcel() {
    Toast.info('Generating', 'Creating Excel file...');

    try {
      if (!window.XLSX) {
        Toast.error('Missing', 'Excel library not loaded');
        return;
      }
      ...
      const myRows = [
        ['Date', 'Description', 'Category', 'Type', 'Amount', 'Payment', 'Tags'],
        ...myTxns.map(tx => [
          tx.date,
          tx.description || '',
          getCatLabel(tx.category),
      ...
      const compareRows = [
        ...
        ...[...allCats]
          .sort((a, b) => {
            return ((myCatSpend[b] || 0) + (herCatSpend[b] || 0))
        ...
        [],
        ['TOTAL', mySpend, herSpend, mySpend + herSpend, mySpend - herSpend],
        ['INCOME', myIncome, herIncome, myIncome + herIncome, myIncome - herIncome]
      ];
      ...
      // ── Sheet 4: Budgets ──
      const budgetRows = [
        ['Category', 'Budget', 'Spent', 'Remaining', 'Used %', 'Alert At'],
        ...budgets.map(b => {
      ...
      Toast.error('Failed', 'Could not generate Excel file');
    }
  }
  ...
  async function exportChartImage(canvasId, fileName = 'chart') {
    ...
    if (!canvas) {
      Toast.error('Not found', 'Chart not available');
      return;
    }
    ...
    try {
      ...
      link.href     = dataUrl;
      link.click();
      Toast.success('Saved', 'Chart image downloaded');
    } catch (err) {
      console.error('Chart export error:', err);
      Toast.error('Failed', 'Could not save chart');
    }
  }
